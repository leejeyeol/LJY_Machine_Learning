...
def load_GT():
    ...
    global GT_list
    global check_list
    file_name = askopenfilename(initialdir=os.path.dirname(save_path))
    if os.path.basename(file_name).split('_')[-1].split('.')[0] == "GT":
        if int(os.path.basename(file_name).split('_')[-2]) == len(frame_path_list):
...
def select_frames_path():
    ...
    global GT_list
    global check_list
    dir_path = askdirectory(initialdir="/media/leejeyeol/74B8D3C8B8D38750/Data/endoscope")
    frame_path_list = glob.glob(os.path.join(dir_path, "*.*"))
    frame_path_list.sort()
    save_path = os.path.join(os.path.dirname(dir_path),
                             os.path.basename(dir_path) + "_%d_GT.npy" % len(frame_path_list))
...
def show_image():
    ...
    if panel_cur is None or panel_next is None or panel_past is None or cur_frame_label is None or panel_progress is None:
        ...
        cur_frame_label=tki.Label(root, text="Current frame : %d / %d" % (i_frame+1, len(frame_path_list))).grid(row=0, column=0)
    else:
        ...
        cur_frame_label.configure(text="Current frame : %d / %d" % (i_frame+1, len(frame_path_list)))
...
select_file_btn = tki.Button(root, text="Go to frame", command=go_to_frame)

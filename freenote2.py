...
celebAInterEyeDistance = distance.euclidean((68,111),(107,112))
...
for i in range(1,10000):
    ...
    if len(faces) == 1:
        ...
        interEyeDistance = distance.euclidean(left_eye_mean, right_eye_mean)

        imageAlignRatio = celebAInterEyeDistance/interEyeDistance


        print("%d" % i)
    elif len(faces) == 0:
        resized_input = resize(input, (input.shape[0]*2, input.shape[0]*2))
        ...
        for i_face, face in enumerate(faces):
            ...
            left_eye_mean = ([preds[36:42, 0].mean(), preds[36:42, 1].mean()])
            right_eye_mean = ([preds[42:48, 0].mean(), preds[42:48, 1].mean()])
            interEyeDistance = distance(left_eye_mean, right_eye_mean)

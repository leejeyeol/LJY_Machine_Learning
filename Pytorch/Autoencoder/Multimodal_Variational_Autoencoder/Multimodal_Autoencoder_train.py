...
import Autoencoder.Multimodal_Variational_Autoencoder.Multimodal_Autoencoder_model as model
import Autoencoder.Multimodal_Variational_Autoencoder.Multimodal_Autoencoder_dataloader as dset
...
criterion_BCE = nn.BCELoss(size_average= False)
criterion_D = RMSEloss()

def Variational_loss(input, target, mu, logvar):
    recon_loss = criterion_BCE(input, target)
...
for epoch in range(options.iteration):
    for i, (R, G, B, D, D_mask) in enumerate(dataloader, 0):
       ...
       # if random.random() < 1 / 3:
        ...
        z_R, mu_R, logvar_R = encoder_R(input_R_noise)
        z_G, mu_G, logvar_G = encoder_G(input_G_noise)
        z_B, mu_B, logvar_B = encoder_B(input_B_noise)
        z_D, mu_D, logvar_D = encoder_D(input_D_noise)
        ...
        err_R = Variational_loss(output_R, original_R, mu_R, logvar_R)
        err_R.backward(retain_graph=True)

        err_G = Variational_loss(output_G, original_G, mu_G, logvar_G)
        err_G.backward(retain_graph=True)

        err_B = Variational_loss(output_B, original_B, mu_B, logvar_B)
        err_B.backward(retain_graph=True)

        err_D = Variational_loss(output_D, original_D, mu_D, logvar_D)
        err_D.backward(retain_graph=True)

        optimizer.step()
        LJY_utils.Depth_evaluation(original_D, output_D, D_mask)
        # visualize
        ...
        if True:
            RGB_Image = torch.cat(((torch.cat((R[0], G[0], B[0]), 0)),
                                   (torch.cat((output_R[0].data, output_G[0].data, output_B[0].data), 0)).cpu()),
                                  1)
            Depth_Image = torch.cat(((original_D[0].data).cpu(),(input_D[0].data).cpu(), (output_D[0].data).cpu()), 1)
